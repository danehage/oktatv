...
const videoMapping: Record<string, { category: VideoCategory; tags: string[]; department: string }> = {
  '1096708225': { 
...
export function useVimeoVideos() {
  ...
  const fetchVimeoData = async () => {
    try {
      ...
      setFetchProgress({ fetched: data.totalFetched, total: data.totalRequested });

      // Transform Vimeo data into our Video format
      const transformedVideos: Video[] = data.videos.map((vimeoVideo, index) => {
        ...
        return {
          ...
          thumbnail: vimeoVideo.thumbnail || `https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=800&h=450&fit=crop`,
      ...
      console.log(`Successfully loaded ${transformedVideos.length} videos with real Vimeo data`);
